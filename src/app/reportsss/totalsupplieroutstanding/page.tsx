...
export default function TotalSupplierOutstanding() {
  ...
  const generateReport = async () => {
    ...
    setError(null)
    try {
      const url = `${process.env.NEXT_PUBLIC_API_BASE_URL}suppliers/outstanding/?asOnDate=${asOnDate}`
      ...
      console.error('Error fetching total supplier outstanding:', error)
  ...
  const calculateNetOutstanding = (supplier: SupplierOutstanding) => {
    return supplier.totalAmountCr - supplier.totalAmountDr
  }

  const calculateTotalNetOutstanding = () => {
    return outstandingData.reduce(
      (total, supplier) => total + calculateNetOutstanding(supplier),
      0
    )
  }
      ...
      <header className={`${isDarkMode ? 'bg-gray-800' : 'bg-white'} shadow-sm sticky top-0 z-10`}>
        <div className="container mx-auto px-4 py-4 flex justify-between items-center">
          <div className="flex items-center">
            ...
            <h1 className="text-2xl font-bold">Total Supplier Outstanding</h1>
          </div>
            ...
            <div className="overflow-x-auto">
              <table className={`min-w-full divide-y ${isDarkMode ? 'divide-gray-700' : 'divide-gray-200'}`}>
                ...
                <tbody className={`divide-y ${isDarkMode ? 'divide-gray-700' : 'divide-gray-200'}`}>
                  {outstandingData.map((supplier, index) => (
                    <tr key={index} className={isDarkMode ? 'hover:bg-gray-700' : 'hover:bg-gray-50'}>
                      ...
                      <td className={`px-6 py-4 whitespace-nowrap text-sm text-right ${isDarkMode ? 'text-gray-300' : 'text-gray-900'}`}>
                        {calculateNetOutstanding(supplier).toFixed(2)}
                      </td>
                    </tr>
                  ))}
                </tbody>
                <tfoot>
                  <tr>
                    <td className={`px-6 py-4 font-bold text-right ${isDarkMode ? 'text-gray-300' : 'text-gray-900'}`}>Total Net Outstanding:</td>
