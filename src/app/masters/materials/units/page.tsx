...
import { Moon, Sun, Plus, Edit, Trash2, Calendar, FileText, Database, Ruler, ArrowLeft } from 'lucide-react'
...
export default function UnitsPage() {
  ...
  const fetchUnits = async () => {
    try {
      setLoading(true)
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}units/`)
      if (response.ok) {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setErrorMessage(errorData.errors?.name?.[0] || 'Failed to create unit')
      }
    } catch (error) {
  ...
  const handleEdit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}units/${editingUnit.id}/`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (response.ok) {
        ...
        const errorData = await response.json()
        setErrorMessage(errorData.errors?.name?.[0] || 'Failed to update unit')
      }
    } catch (error) {
  ...
  const handleDelete = async (id: number, name: string) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}units/${id}/`, {
        method: 'DELETE',
      })
      ...
      setErrorMessage('Error deleting unit')
      console.error('Error:', error)
    }
  }
          ...
          <div className="p-6">
            {loading ? (
              ...
              <div className="space-y-2">
                {units.map((unit) => (
                  <div key={unit.id} className={`flex items-center justify-between p-3 rounded-lg ${isDarkMode ? 'hover:bg-gray-700' : 'hover:bg-gray-50'} transition-colors`}>
              ...
              <form onSubmit={handleEdit} className="space-y-4">
                ...
                <div className="flex justify-end space-x-3 pt-4">
                  ...
                  <button
                    ...
                    className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-medium disabled:opacity-50"
                  >
                    {submitting ? 'Updating...' : 'Update Unit'}
                  </button>
                </div>
              </form>
            </div>
