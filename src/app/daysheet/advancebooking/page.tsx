...
export default function AdvanceBooking() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}advance-booking/`, {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        throw new Error('Failed to submit advance booking')
      }

      const result = await response.json()
      console.log('Advance booking submitted:', result)
      ...
      console.error('Error submitting advance booking:', error)
      setError('Failed to submit advance booking. Please try again.')
    } finally {
      setIsLoading(false)
    }
  }
