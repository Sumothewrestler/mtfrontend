...
export default function CategoryWiseExpenseReport() {
  ...
  const generateReport = async () => {
    ...
    setError(null)
    try {
      const url = `${process.env.NEXT_PUBLIC_API_BASE_URL}expenses/category-report/?fromDate=${fromDate}&toDate=${toDate}`
      ...
      <header className={`${isDarkMode ? 'bg-gray-800' : 'bg-white'} shadow-sm sticky top-0 z-10`}>
        <div className="container mx-auto px-4 py-4 flex justify-between items-center">
          <div className="flex items-center">
            ...
            <h1 className="text-2xl font-bold">Category-wise Expense Report</h1>
          </div>
              ...
              <table className={`min-w-full divide-y ${isDarkMode ? 'divide-gray-700' : 'divide-gray-200'}`}>
                ...
                <tfoot className={isDarkMode ? 'bg-gray-700' : 'bg-gray-50'}>
                  <tr>
                    ...
                    <td className={`py-2 px-4 whitespace-nowrap text-sm font-medium text-right ${isDarkMode ? 'text-gray-300' : 'text-gray-900'}`}>{totalExpense.toFixed(2)}</td>
                  </tr>
                </tfoot>
              </table>
            </div>
