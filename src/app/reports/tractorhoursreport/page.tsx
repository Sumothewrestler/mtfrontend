      ...
      <div className="flex-1 overflow-auto">
        <header className={`${isDarkMode ? 'bg-gray-800' : 'bg-white'} shadow-sm flex justify-between items-center px-6 py-4`}>
          <div className="flex items-center">
            ...
            <h1 className="text-2xl font-semibold">Tractor Hours Report</h1>
          </div>
                ...
                <table className={`min-w-full divide-y ${isDarkMode ? 'divide-gray-700' : 'divide-gray-200'}`}>
                  <thead className={isDarkMode ? 'bg-gray-700' : 'bg-gray-50'}>
                    <tr>
                      ...
                      {tractors.map((tractor) => (
                        <th key={tractor.id} className={`py-2 px-4 text-left text-xs font-medium uppercase tracking-wider ${isDarkMode ? 'text-gray-300' : 'text-gray-500'}`}>
                          {tractor.name}
                        </th>
                      ))}
                    </tr>
                  ...
                  <tbody className={`divide-y ${isDarkMode ? 'divide-gray-700' : 'divide-gray-200'}`}>
                    {Array.from({ length: getDaysInMonth(month, year) }, (_, i) => i + 1).map((day) => (
                      <tr key={day} className={isDarkMode ? 'hover:bg-gray-700' : 'hover:bg-gray-50'}>
                        <td className={`py-2 px-4 whitespace-nowrap text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-900'}`}>{day}</td>
                        {tractors.map((tractor) => (
                          <td key={tractor.id} className={`py-2 px-4 whitespace-nowrap text-sm text-center ${isDarkMode ? 'text-gray-300' : 'text-gray-500'}`}>
                  ...
                  <tfoot className={isDarkMode ? 'bg-gray-700' : 'bg-gray-50'}>
                    <tr>
                      <td className={`py-2 px-4 whitespace-nowrap text-sm font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-900'}`}>Total Hours</td>
                      {tractors.map((tractor) => (
                        <td key={tractor.id} className={`py-2 px-4 whitespace-nowrap text-sm text-center ${isDarkMode ? 'text-gray-300' : 'text-gray-500'}`}>
